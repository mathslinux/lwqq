...
GType qq_mainpanel_get_type()
{
    ...
    if (!t) {
        ...
        t = g_type_register_static(GTK_TYPE_VBOX, "QQMainPanel", &info, 0);
    }
    return t;
}
...
/* FIXME: static */
void qq_group_list_on_double_click(GtkTreeView *tree,
                                          GtkTreePath *path,
                                          ...
                                          gpointer data)
{
    ...
    g_debug("Create group chat window for %s(%s, %d)",
...
//
/* FIXME: static */
gboolean qq_group_list_on_show_tooltip(GtkWidget* widget,
                                       int x,
                                       int y,
                                       ...
                                       gpointer data)
{
    ...
    gtk_tree_model_get(model, &iter, BDY_LIST_NAME, &name,
                       BDY_LIST_UIN, &code, -1);
...
void qq_mainpanel_init(QQMainPanel *panel)
{
    ...
    gtk_box_pack_start(GTK_BOX(panel->longnick_box),
                       panel->longnick_eventbox, FALSE, FALSE, 0);
    gtk_box_pack_start(GTK_BOX(panel->longnick_box),
                       panel->longnick_entry, FALSE, FALSE, 0);
    gtk_box_pack_start(GTK_BOX(vbox), panel->longnick_box,
                       FALSE, FALSE, 0);
...
static gboolean lnick_release_cb(GtkWidget *w, GdkEvent *event, gpointer data)
{
    ...
    gtk_widget_show(GTK_WIDGET(panel->longnick_entry));
    gtk_widget_set_size_request(GTK_WIDGET(panel->longnick_entry), -1, 22);
...
static gboolean lnick_enter_cb(GtkWidget *w, GdkEvent *event, gpointer data)
{
    ...
    GdkColor color;
    gdk_color_parse("#C4E6FF", &color);
    gtk_widget_modify_bg(GTK_WIDGET(panel->longnick_eventbox), GTK_STATE_NORMAL, &color);
    return TRUE;
}
...
static void update_my_face_image(QQMainPanel *panel)
{
    ...
    GError *err = NULL;
    /* FIXME */
    gchar buf[500] = "/usr/share/lwqq/icons/webqq.png";
